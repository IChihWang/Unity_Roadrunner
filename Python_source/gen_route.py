...
def generate_routefile(arrival_rate):
    ...
    with open("data/Roadrunner_net.rou.xml", "w") as routes:
        print("<routes>\n", file=routes)

        vType_str = '\t<vType id="car%i" accel="100.0" decel="100.0" sigma="0.0" length="%f" minGap="%f" maxSpeed="%f" tau="%f" carFollowModel="CACC" color="255,255,255"/>' % (0, 6.0, cfg.HEADWAY, cfg.MAX_SPEED, cfg.TIME_STEP);
        ...
        for i in range(cfg.N_TIME_STEP):
            for idx in range(4):
                if random.uniform(0, 1) < dir_prob[idx]:
                    ...
                    veh_str += '_%i" type="car%i" route="route%s" depart="%i" departLane = "%i" departSpeed="%f"/>' % (vehNr, 0, str(idx+1)+'_'+str((idx+dir_r)%4+1), i, lane_r, cfg.MAX_SPEED);
                    idl += '_%i'% (vehNr);
                    idllist.append(idl)
